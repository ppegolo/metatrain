...
only valid if the augmenter's Wigner convention matches how PySCF's real
spherical auxiliary functions transform, i.e. (with D the block-diagonal
per-shell Wigner matrix in PySCF AO order):
...
def _augmenter_wigner_matrices(rotation: Rotation, lmax: int) -> dict[int, np.ndarray]:
    """Real Wigner matrices built exactly like RotationalAugmenter builds them."""
    ...
    q = _scipy_quaternion_to_quaternionic(rotation.as_quat())
    dcs = wigner.D(q)
    out = {}
    for ell in range(lmax + 1):
        U = _complex_to_real_spherical_harmonics_transform(ell)
        wc = np.zeros((2 * ell + 1, 2 * ell + 1), dtype=np.complex128)
        for mp in range(-ell, ell + 1):
            for m in range(-ell, ell + 1):
                wc[m + ell, mp + ell] = dcs[wigner.Dindex(ell, m, mp)].conj()
        w = U.conj() @ wc @ U.T
