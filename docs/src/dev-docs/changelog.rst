...
- Standalone training of the ``composition`` architecture now works in distributed
  environments (detected automatically from the SLURM environment, as for the other
  architectures), fits on the requested device instead of always on the CPU, and
  loads data in parallel (``num_workers``). The trainer checkpoint version was
  bumped to 2.
