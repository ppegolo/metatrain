...
def _get_raw_model(model: Union[NEP, DistributedDataParallel], is_distributed: bool):
    """Unwrap a possibly-DDP-wrapped model to get the underlying NEP."""
    return model.module if is_distributed else model


def _compute_q_scaler(
    model: NEP,
    train_datasets: List[Union[Dataset, torch.utils.data.Subset]],
    device: torch.device,
    dtype: torch.dtype,
) -> None:
    """Set the GPUMD-style descriptor normalisation from the training set.

    GPUMD normalises each descriptor dimension by ``1 / (max - min)`` over the
    training data; here the extrema are accumulated over all training
    structures.  Only applied to freshly initialised models (``q_scaler`` all
    ones); restarted models keep their stored normalisation.
    """
    potential = model.potential
    if not bool(torch.all(potential.q_scaler == 1.0)):
        return

    logging.info("Computing NEP descriptor normalisation (q_scaler)")
    q_min: torch.Tensor | None = None
    q_max: torch.Tensor | None = None
    for dataset in train_datasets:
        for sample in dataset:
            system = sample["system"].to(device=device, dtype=dtype)
            sample_min, sample_max = model.descriptor_extrema([system])
            q_min = sample_min if q_min is None else torch.minimum(q_min, sample_min)
            q_max = sample_max if q_max is None else torch.maximum(q_max, sample_max)

    if q_min is None or q_max is None:
        return
    q_range = (q_max - q_min).clamp_min(1.0e-10)
    with torch.no_grad():
        potential.q_scaler.copy_((1.0 / q_range).to(potential.q_scaler.dtype))
...
class Trainer(TrainerInterface[TrainerHypers]):
    ...
    def train(
        ...
        assert dtype in NEP.__supported_dtypes__
        ...
        if is_distributed:
            if len(devices) > 1:
                raise ValueError(
                    ...
                    " If you want to run distributed training with NEP, please "
                    "set `device` to cuda."
                )
        else:
            device = devices[
