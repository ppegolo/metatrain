...
class GLE(ModelInterface[ModelHypers]):
    ...
    def __init__(self, hypers: ModelHypers, dataset_info: DatasetInfo) -> None:
        ...
        self._add_output(
            ...
            TargetInfo(
                layout=TensorMap(
                    ...
                    blocks=[
                        TensorBlock(
                            values=torch.empty(0, self.n_gle_variables**2),
                            samples=Labels(
                                names=["system", "atom"],
                            ...
                            properties=Labels(
                                names=["A"],
                                values=torch.arange(
                                    self.n_gle_variables**2, dtype=torch.long
                                ).unsqueeze(1),
                            ),
                        )
                    ],
        ...
        # Added to the raw mtt::A output in forward (see _get_output_atomic_predictions)
        # so the training loss and the exported deploy model see the same theta_total.
        # Zeros by default => no baseline, backward-compatible. Indexed by atomic number,
        # persistent so it exports self-contained with the model.
        ...
        if self.hypers.get("zero_init_readout", False):
            for last_layers in (self.node_last_layers, self.edge_last_layers):
    ...
    def forward(
        ...
        **Post-processing (Evaluation Only)**
        ...
        outputs["mtt::A"] = ModelOutput(per_atom=True)
    ...
    def _get_output_atomic_predictions(
        ...
        :param system_indices: Tensor mapping each atom to its system index
            [n_atoms].
        :param sample_labels: Labels for all atoms in the batch [n_atoms, 2].
        :param species: Atomic number of each atom in the batch [n_atoms], used to
            add the per-type frozen theta baseline to the ``mtt::A`` prediction.
        :param outputs: Dictionary of requested outputs.
        :param selected_atoms: Optional Labels specifying a subset of atoms to include.
        ...
        for output_name in self.target_names:
            if output_name in outputs:
                ...
                # Delta-learning: add the frozen per-type theta baseline to the raw
                # mtt::A output, so make_A (applied in the loss and at deploy) sees
                # theta_total = theta_net(Q) + theta_base[Z]. Zeros baseline => no-op.
                if output_name == "mtt::A":
                    theta_base = self.theta_baseline.index_select(0, species).to(
                        edge_vectors.dtype
                    )
                    for key in atomic_predictions_by_block:
                        atomic_predictions_by_block[key] = (
                            atomic_predictions_by_block[key] + theta_base
                        )
