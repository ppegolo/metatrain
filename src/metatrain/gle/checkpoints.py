...
def model_update_v13_v14(checkpoint: dict) -> None:
    ...
    Adds the ``theta_baseline`` buffer (per-type frozen Delta-learning baseline for
    the GLE drift). v13 models had no baseline, so it is injected as zeros, which is
    a no-op for the drift (``theta_total = theta_net + 0``).

    :param checkpoint: The checkpoint to update.
    """
    model_hypers = checkpoint["model_data"]["model_hypers"]
    n_gle = 3 + int(model_hypers["num_auxiliary_variables"])
    atomic_types = checkpoint["model_data"]["dataset_info"].atomic_types
    zeros = torch.zeros(max(atomic_types) + 1, n_gle**2)
    for key in ("model_state_dict", "best_model_state_dict"):
        state_dict = checkpoint.get(key)
        if state_dict is not None and "theta_baseline" not in state_dict:
            state_dict["theta_baseline"] = zeros.clone()
...
def model_update_v15_v16(checkpoint: dict) -> None:
    ...
    Adds the system-conditioning hyperparameters that the shared PET backend now
    expects, disabled by default so existing models keep their behaviour.
...
def model_update_v16_v17(checkpoint: dict) -> None:
    ...
    The learnable modules (GNN layers, embedders, heads, last layers and the
    ``species_to_species_index`` buffer) moved into the shared ``PETBackend``
    submodule, registered as ``self.backend``. This prefixes the affected
    ``state_dict`` keys with ``backend.``, leaving ``theta_baseline``, the additive
    models, the scaler, the long-range featurizer and ``finetune_config`` untouched.
...
def trainer_update_v13_v14(checkpoint: dict) -> None:
    ...
    :param checkpoint: The checkpoint to update.
    """
    # Replace the scalar ``bead_mass`` hyper with per-type ``bead_mass_by_symbol``.
    # v13 checkpoints are water-only (a single "O"-tagged bead type).
    bead_mass = checkpoint["train_hypers"].pop("bead_mass")
    checkpoint["train_hypers"]["bead_mass_by_symbol"] = {"O": bead_mass}


def trainer_update_v14_v15(checkpoint: dict) -> None:
    """
    Update trainer checkpoint from version 14 to version 15.

    The ``batch_atom_bounds`` field has been removed from the trainer schema
    (max-atom packing is now done by the sampler). ``batch_atom_bounds`` was
    already inert whenever ``max_atoms_per_batch`` was set (packing disabled the
    post-hoc filter), so its bounds are only translated into the equivalent
    ``max_atoms_per_batch`` / ``min_atoms_per_batch`` sampler settings when they
    were the mechanism actually in effect; otherwise the field is simply dropped.
