...
def _worker_init_disable_autograd_threads(worker_id: int) -> None:
    """DataLoader worker initializer: make the autograd backward engine
    single-threaded inside each (forked) worker. The remove-additive collate
    transform runs torch.autograd for position-dependent additive forces (ZBL,
    SoftCore); on some torch builds a multithreaded backward engine cannot run
    in a fork-based worker ("Unable to handle autograd's threading in
    combination with fork-based multiprocessing"). Disabling it here, before the
    worker performs any backward, is fork-safe. No-op when there is no additive.
    """
...
class Trainer(TrainerInterface[TrainerHypers]):
    ...
    def train(
        ...
        # When an additive model contributes position-dependent forces (e.g. ZBL
        # or SoftCore), the remove-additive collate transform runs torch.autograd
        # inside the (fork-based) DataLoader workers. On some torch builds the
        # autograd backward engine's worker threads cannot be inherited across
        # fork ("Unable to handle autograd's threading in combination with
        # fork-based multiprocessing"). Disabling autograd multithreading makes
        # the backward engine single-threaded, which is fork-safe; spawn is not
        # an option because the collate transforms are not picklable.
        dl_worker_init = (
            _worker_init_disable_autograd_threads
