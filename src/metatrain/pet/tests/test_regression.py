...
def _make_ri_dataset_and_target_info() -> tuple[Dataset, str, DatasetInfo]:
    ...
    targets = [
        _make_constant_ri_target(system, i_system, target_info.layout, value=0.1)
        for i_system, system in enumerate(systems)
    ]
    projections = [
        _make_constant_ri_target(system, i_system, target_info.layout, value=0.05)
        for i_system, system in enumerate(systems)
    ]
...
def test_regression_density_overlap_train_runs(tmp_path):
    ...
    eval_system = get_system_with_neighbor_lists(
        systems[0].to(torch.float32), model.requested_neighbor_lists()
    )
    output = evaluate_model(
        model,
        [eval_system],
        targets={target_name: ModelOutput(quantity="", unit="", sample_kind="atom")},
        is_training=False,
    )
    assert target_name in output
    # A wrong or NaN loss leaves the output key present but poisons the weights
    # after the training step above: require every predicted value to be finite.
    for block in output[target_name].blocks():
    ...
    hypers["training"]["loss"] = OmegaConf.create(
        {
            target_name: {
                ...
                "reduction": "mean",
                "gradients": {},
                "projections_key": "mtt::ri_projections",
            }
        }
    )
