...
class PET(ModelInterface[ModelHypers]):
    ...
    def __init__(self, hypers: ModelHypers, dataset_info: DatasetInfo) -> None:
        ...
        # has no such key, and bracket access killed the restart with
        # `KeyError: 'soft_core'` (3 metatrain tests). Same reason the per-type keys
        # below use `.get`.
        if self.hypers.get("soft_core", False):
            ...
            # `.get` (rather than `[...]`) so that checkpoints written before the
            # per-type/masking hypers existed still restart.
            additive_models.append(
                SoftCore(
                    {
                        "sigma_by_type": self.hypers.get(
                            "soft_core_sigma_by_type", {}
                        ),
        ...
        if self.hypers.get("harmonic_bonded", False):
            ...
            # `.get` (rather than `[...]`) so that checkpoints written before
            # these hypers existed still restart.
            additive_models.append(
                HarmonicBonded(
                    {
