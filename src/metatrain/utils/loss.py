...
def _split_ensemble_members(
    ...
    """Recover the ensemble axis of an LLPR ensemble block. Component dimensions
    sit between samples and properties, so the ensemble axis must be recovered
    from the *last* dimension, not from dimension 1.

    :param ensemble_values: values of shape
        ``(samples, *components, n_ens * n_properties)``.
    :param n_properties: number of properties of the corresponding target block.
    :return: the values reshaped to ``(samples, *components, n_ens, n_properties)``.
    """
