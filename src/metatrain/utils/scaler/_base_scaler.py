...
class BaseScaler(torch.nn.Module):
    ...
    def fit(
        ...
        # Non-finite scales can arise if a target has zero samples in the training
        # data, and zero scales can arise for constant targets. Use a neutral scale in
        # both cases to avoid divisions by zero during training.
        for target_name in targets_to_fit:
            for key in self.scales[target_name].keys:
                scales = self.scales[target_name][key].values
                scales[~torch.isfinite(scales) | (scales == 0.0)] = 1.0

                per_target_scales = self.per_target_scales[target_name][key].values
                per_target_scales[
                    ~torch.isfinite(per_target_scales) | (per_target_scales == 0.0)
                ] = 1.0
    ...
    def fit_per_property(
        ...
        for target_name in targets_to_fit:
            ...
            for key in self.per_property_N[target_name].keys:
                ...
                # If any scales are zero or non-finite, set them to 1.0
                block.values[
                    ~torch.isfinite(block.values) | (block.values == 0.0)
                ] = 1.0
