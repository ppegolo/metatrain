...
class Readout(torch.nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(layer_dims) - 1):
            ...
            # Initialise each species slice exactly like ``torch.nn.Linear``:
            # on the full 3-D tensor, kaiming_uniform_ would compute
            # fan_in = out_dim * in_dim instead of in_dim, shrinking the
            # init scale by ~sqrt(out_dim).
            for s in range(actual_n_species):
                torch.nn.init.kaiming_uniform_(w[s], a=math.sqrt(5))
...
class LinearReadout(torch.nn.Linear):
    """
    Plain shared linear last layer for non-atomic-basis targets.

    Subclasses :class:`torch.nn.Linear` so parameter names, shapes and
    initialisation (``weight`` of shape ``(out, in)``, ``bias``) are identical
    to the plain ``torch.nn.Linear`` last layers used before atomic-basis
    readouts existed — existing checkpoints and LLPR's named-parameter lookups
    keep working. The extra ``species_idx`` argument is accepted and ignored
    so the module is call-compatible with the Z-conditioned readouts.
    """
    ...
    def forward(  # type: ignore[override]
        ...
        """
        :param features: Input features of shape ``(..., in_features)``.
        :param species_idx: Ignored; present for call compatibility with the
            species-conditioned readouts.
        :return: Output features of shape ``(..., out_features)``.
        """
...
class MoEReadout(torch.nn.Module):
    ...
    Replaces a single ``Readout`` with a pool of N lightweight
    ...
    * **Shared experts** (N − I): always active with unit weight, capturing
    ...
    Each expert f_j is a ``Readout(z_conditioned=False)``; the
    ...
    **TorchScript note**: all I routed experts are evaluated for every atom
    ...
    :param hidden_layer_widths: Optional hidden widths forwarded to each
        expert ``Readout``; ``None`` gives linear experts.
    ...
    def __init__(
        ...
        if num_routed_experts < 1:
            raise ValueError(
                ...
                "Use Readout directly when no Z-gating is needed."
            )
        ...
        # n_species=1 is passed but ignored by Readout when
...
class IrrepResidualReadout(torch.nn.Module):
    ...
    * **trunk** is a :class:`Readout` — the standard per-species
    ...
    * ``d loss / d MLP params`` = ``gate * ...`` → zero initially, but the gate
    ...
    :param z_conditioned: Passed through to the trunk
        :class:`Readout`.  If ``True``, the trunk uses per-species
...
class IrrepResidualZCorrectionDeep(torch.nn.Module):
    ...
    **Recommended sweep** — ``num_correction_layers`` ∈ {1, 2, 3} at each
    ...
    :param num_correction_layers: Number of hidden layers ``K`` in the
        correction tower.  Default ``2``. The ``IrrepResidualZCorrection``
        config name is an alias for ``K = 1``.
    :param expansion_factor: Width multiplier for the correction hidden
        layers (hidden dimension = ``in_features * expansion_factor``).
        Default ``1``.
    """
    ...
    def __init__(
        ...
        self.correction = Readout(
            in_features,
            out_features,
            n_species,
            z_conditioned=True,
            hidden_layer_widths=(
                [in_features * expansion_factor] * num_correction_layers
            ),
        )
...
class IrrepThenZConditioned(torch.nn.Module):
    ...
    2. **Z-conditioned readout** ``d'(α) → q(α, Z)`` — a
       :class:`Readout` (optionally with further hidden layers)
    ...
    :param in_features: Input feature dimension ``d`` (``d_head``). The irrep
        MLP maps ``d → d`` (the hidden dimension equals ``in_features``).
    ...
    :param hidden_layer_widths: Optional additional hidden layers inside the
        Z-conditioned readout (forwarded to :class:`Readout`).
    """
