...
def _get_block(tensor_map: TensorMap, key: LabelsEntry) -> TensorBlock:
    """``tensor_map[key]``, resolving the block position on CPU.

    Indexing a TensorMap on CUDA by key fails in metatensor-torch <= 0.9.1
    (the internal selection is created on CPU; fixed upstream in 0.10), so
    resolve the position with CPU labels and index by block number instead.
    """
    position = tensor_map.keys.to("cpu").position(key.values.cpu())
    if position is None:
        raise KeyError(f"key {key} not found in TensorMap")
    return tensor_map.block(position)
    ...
    def accumulate(
        ...
        for target_name, target in targets.items():
            for key, block in target.items():
                ...
                _get_block(self.XTX[target_name], key).values[:] += XTX

                # Compute and accummulate "XTY", i.e. X.T @ Y
                XTY = _get_block(self.XTY[target_name], key)
    ...
    def fit(
        ...
        for target_name in targets_to_fit:
            ...
            for key in self.XTX[target_name].keys:
                XTX_block = _get_block(self.XTX[target_name], key)
                XTY_block = _get_block(self.XTY[target_name], key)
