...
def resolve_dataloader_workers(
    ...
    The worker count is chosen automatically when ``requested_num_workers`` is
    ``None`` (see :func:`metatrain.utils.data.get_num_workers`) and validated
    otherwise. Workers are started with the ``spawn`` method when training on
    CUDA — forking after CUDA initialization is unsupported by PyTorch, and
    forked workers gradually copy the training process's memory — but only if
    every dataset is disk-backed: spawned workers receive their dataset by
    pickling, which would copy an in-memory dataset into every worker.
    ...
    :return: A tuple of the number of workers and the multiprocessing context
        to pass to the dataloader builders (``"spawn"`` or ``None``).
    """
...
def datasets_are_disk_backed(datasets: List[DatasetLike]) -> bool:
    """Whether every dataset reads its samples from disk.

    Disk- and memmap-backed datasets pickle to a few kB (paths and offsets),
    so spawned dataloader workers receive them essentially for free. An
    in-memory dataset is fully copied into every spawned worker instead;
    callers use this to decide whether "spawn" workers are worth it.
...
def ensure_spawn_safe_sharing(multiprocessing_context: Optional[str]) -> None:
    ...
    With spawned dataloader workers, the dataset and every batch are pickled
    across the worker boundary. PyTorch's default ``"file_descriptor"``
    sharing strategy passes tensors as open file descriptors, which spawned
    subprocesses cannot inherit (``ValueError: bad value(s) in fds_to_keep``),
    and some container setups additionally restrict ``/dev/shm``, on which
    that strategy relies. The ``"file_system"`` strategy uses regular files
    instead, which works in both cases.

    :param multiprocessing_context: The worker start method about to be used;
        only ``"spawn"`` requires the switch.
    """
