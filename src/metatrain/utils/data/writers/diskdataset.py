...
class DiskDatasetWriter(Writer):
    ...
    def __init__(
        ...
        if self._append:
            ...
            # them at "0/", "1/", ... The zip may have gaps (entries missing due
            # to failed structure processing), so continue after the highest
            # entry number rather than the entry count. Also pick up an existing
            # atom-count file (if any and not stale) so it can be extended in
            # finish() rather than dropped.
            try:
    ...
    def finish(self) -> None:
        ...
        In append mode, the file is extended when a previous session already had
        one (see ``__init__``); otherwise it is skipped, since this writer has no
        way to know the atom counts of entries it didn't write itself.
        """
        ...
        if not self._append:
            ...
            # Zip files don't support in-place replacement of an entry: this adds
            # a second "_atom_counts.npy" rather than overwriting the first. It is
            # harmless, as reading a duplicated name returns the last one written,
            # so the old, superseded array is just dead weight in the archive.
